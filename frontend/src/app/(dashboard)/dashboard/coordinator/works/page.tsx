...
import { thesisApi, careersApi, paymentsApi } from '@/lib/api';
...
export default function CoordinatorWorksPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['coordinator-works', search, status, careerId, page],
    queryFn: () => thesisApi.list({ search, status, careerId, page, limit: 15 }),
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {data?.total ?? 0} trabajo{data?.total !== 1 ? 's' : ''} en total
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Download className="w-4 h-4" /> Exportar trabajos
          </button>
          <button
            onClick={() => paymentsApi.exportCsv()}
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50"
          >
            <Download className="w-4 h-4" /> Exportar pagos
          </button>
        </div>
      </div>
                ...
                <tr key={w.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <Link
                      ...
                      className="font-medium text-gray-900 hover:text-unphu-700 line-clamp-1"
                    >
                      {w.title}
                    </Link>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {w.type === 'TESIS' ? 'Tesis' : 'Monográfico'} · {w.year}
                    </p>
                  </td>
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {w.advisor ? (
                      <span>{w.advisor.user?.firstName} {w.advisor.user?.lastName}</span>
                    ) : (
