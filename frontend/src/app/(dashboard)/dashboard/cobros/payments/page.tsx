...
import { CreditCard, CheckCircle, XCircle, ExternalLink, Download } from 'lucide-react';
...
export default function CobrosPaymentsPage() {
  ...
  const { data: payments, isLoading } = useQuery({
    queryKey: ['cobros-payments', filter],
  ...
  const confirmMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cobros-payments'] }),
  });
              ...
              {p.status === 'SUBMITTED' && (
                <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => confirmMutation.mutate({ id: p.thesisWorkId, notes: notes[p.id] })}
                    ...
                    <button onClick={() => rejectMutation.mutate({ id: p.thesisWorkId, notes: notes[p.id] })}
